...
4. Limit generacji: egzekwowany na poziomie wejścia do LLM, bez mechanizmu w bazie danych.
5. analytics_events jako tabela uniwersalna (JSONB) + indeksy (event_type, created_at) oraz blokada odczytu przez RLS (opcjonalnie w MVP).
6. Strategia Storage: storage_path w DB, buckety/prefiksy per użytkownik, polityki dostępu oparte o auth.uid().
...
2. Upload zdjęć per pomieszczenie, z rozróżnieniem: zdjęcia „pomieszczenia” i „inspiracji”.
3. Generowanie wariantów dla pojedynczego pomieszczenia: każdy wariant ma 2 obrazy i bullet points (sugestie) zwracane w odpowiedzi API.
4. Limit generacji: kontrolowany przez kredyty dostępne w LLM, bez twardego limitu w aplikacji.
5. Analityka: opcjonalnie zdarzenie InspirationGenerated (i potencjalnie kolejne w przyszłości).
